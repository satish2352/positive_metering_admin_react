...
const BlogDetails = () => {
  ...
  const fetchTeam = async () => {
    try {
      const response = await axios.get("http://localhost:5000/blogdetails");
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm(formData)) {
      try {
        if (editMode) {
          await axios.put(
            `http://localhost:5000/blogdetails/${editingId}`,
            formData
          );
          toast.success("Data Updated Successfully");
        } else {
          await axios.post("http://localhost:5000/blogdetails", formData);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/blogdetails/${id}`);
