...
import instance from "../../api/AxiosInstance";   // <-- your API instance
import { toast } from "react-toastify";
import { useNavigate } from "react-router-dom";
import logo from "../../assets/images/logo.png";
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      setLoading(true);
      try {
        // 🚀 Make API call with email, password, captcha
        const response = await instance.post(
          "/auth/login",
        ...
        <Col xs={12} sm={10} md={8} lg={6} xl={4}>
          <Card className="shadow-lg border-0 rounded-3">
            <Card.Body className="p-4 p-md-5 bg-white">
              ...
              {loading ? (
                <div className="d-flex justify-content-center align-items-center">
                  <ThreeDots height="80" width="80" radius="9" color="#007bff" />
                </div>
              ) : (
                <Form onSubmit={handleSubmit}>
                  <Form.Group controlId="formBasicEmail" className="mb-4">
                    <Form.Label className="d-flex align-items-center">
                      <FaUser className="me-2 text-secondary" /> Email address
                    </Form.Label>
                    <Form.Control
                      type="email"
                      placeholder="Enter email"
                  ...
                  <Form.Group controlId="formBasicPassword" className="mb-4">
                    <Form.Label className="d-flex align-items-center">
                      <FaLock className="me-2 text-secondary" /> Password
                    </Form.Label>
                    <InputGroup className="bg-light border-0 shadow-sm rounded-pill">
                      <Form.Control
                        type={showPassword ? "text" : "password"}
                  ...
                  <div className="mb-3 d-flex justify-content-center">
                    <ReCAPTCHA
                      sitekey="6LcKvb4rAAAAAAIUQMqYSaiODF6ayGcKNa1ldrgl"
                      onChange={(value) => setCaptchaValue(value)}
                    />
                  </div>
                  {errors.captcha && (
                    <span className="text-danger d-block text-center">{errors.captcha}</span>
                  )}
